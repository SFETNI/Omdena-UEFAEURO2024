...
def get_actions(df_events: pd.DataFrame):
    ...
    df_actions.loc[df_actions["loc_y"] > 79, "loc_x"] = 79
...
def create_actions_grid(df_actions: pd.DataFrame, football_pitch_size, heatmap_tile_size):
    ...
    df_actions_grid = df_actions[["player_id", "Shot", "Pass", "Dribble", "grid_index"]]
    df_actions_grid = df_actions_grid.groupby(["player_id", "grid_index"], as_index=False).sum()
